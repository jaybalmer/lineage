    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted mt-1">Search people, places, and connections across the graph</p>
        </div>
        ...
        {!query && (
          <div className="grid grid-cols-2 gap-4 mb-8">
            {mostConnectedPlace && (
              <CommunityLink href={`/places/${placeSlug(mostConnectedPlace.place)}`}>
                <div className="bg-surface border border-border-default rounded-xl p-4 hover:border-border-default transition-all">
                  <div className="text-xs text-muted mb-1">Most ridden</div>
                  <div className="font-semibold text-foreground">{mostConnectedPlace.place.name}</div>
                  <div className="text-xs text-muted mt-0.5">{mostConnectedPlace.count} riders documented</div>
                </div>
              </CommunityLink>
            )}
            {mostDocumented && (
              <CommunityLink href={`/riders/${mostDocumented.person.id}`}>
                <div className="bg-surface border border-border-default rounded-xl p-4 hover:border-border-default transition-all">
                  ...
                  <div className="text-xs text-muted mt-0.5">{mostDocumented.documented} documented claims</div>
                </div>
              </CommunityLink>
            )}
          </div>
        ...
        {query && (
          <div className="space-y-6">
            ...
            {filteredPlaces.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">Places</div>
                <div className="grid grid-cols-2 gap-2">
                  {filteredPlaces.map((p) => (
                    <CommunityLink key={p.id} href={`/places/${placeSlug(p)}`}>
                      <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                        <span className="text-lg">🏔</span>
                        <div>
                          <div className="text-sm font-medium text-foreground">{p.name}</div>
                          {p.region && <div className="text-xs text-muted">{p.region}</div>}
                        </div>
                      </div>
                    </CommunityLink>
                  ))}
            ...
            {filteredOrgs.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">Brands</div>
                <div className="grid grid-cols-2 gap-2">
                  {filteredOrgs.map((o) => (
                    <CommunityLink key={o.id} href={`/orgs/${orgSlug(o)}`}>
                      <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                        <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center text-xs font-bold text-muted">{o.name[0]}</div>
                        <div>
                          <div className="text-sm font-medium text-foreground">{o.name}</div>
                          {o.founded_year && <div className="text-xs text-muted">est. {o.founded_year}</div>}
                        </div>
                      </div>
                    </CommunityLink>
                  ))}
            ...
            {filteredBoards.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">Boards</div>
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <CommunityLink key={b.id} href={`/boards/${boardSlug(b)}`}>
                      <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted">'{String(b.model_year).slice(2)} · {b.shape ?? "–"}</div>
                        </div>
                      </div>
                    </CommunityLink>
                  ))}
            ...
            {filteredEvents.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">Event Series</div>
                <div className="grid grid-cols-2 gap-2">
                      ...
                      <CommunityLink key={s.id} href={`/events/${seriesSlug(s)}`}>
                        <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted">{instanceCount} years documented</div>
                          </div>
                        </div>
                      </CommunityLink>
                    )
            ...
            {filteredPeople.length === 0 && filteredPlaces.length === 0 && filteredOrgs.length === 0 && filteredBoards.length === 0 && filteredEvents.length === 0 && (
              <div className="text-center text-muted py-12 text-sm">No results for &ldquo;{query}&rdquo;</div>
            )}
          </div>
        )}
        ...
        {!query && (
          <div className="space-y-8">
            {/* Riders */}
            <div>
              <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-4">Riders</div>
            ...
            {/* Brands */}
            <div>
              <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-4">Brands</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {ORGS.filter((o) => o.brand_category).map((o) => {
                  ...
                  return (
                    <CommunityLink key={o.id} href={`/orgs/${orgSlug(o)}`}>
                      <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                        <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center text-xs font-bold text-muted flex-shrink-0">{o.name[0]}</div>
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-foreground truncate">{o.name}</div>
                          <div className="text-xs text-muted">{riderCount > 0 ? `${riderCount} riders` : o.founded_year ? `est. ${o.founded_year}` : ""}</div>
                        </div>
                      </div>
                    </CommunityLink>
                  )
            ...
            {/* Event Series */}
            <div>
              <div className="text-xs font-semibold text-muted uppercase tracking-widest mb-4">Event Series</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {EVENT_SERIES.map((s) => {
                  const instanceCount = EVENTS.filter((e) => e.series_id === s.id).length
                    ...
                    <CommunityLink key={s.id} href={`/events/${s.id}`}>
                      <div className="flex items-center gap-3 p-3 bg-surface border border-border-default rounded-lg hover:border-border-default transition-all">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-muted">{instanceCount} years · {attendeeCount} riders</div>
                        </div>
                      </div>
                    </CommunityLink>
                  )
