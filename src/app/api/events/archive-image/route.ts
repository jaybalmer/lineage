...
/**
 * POST /api/events/archive-image
 *
 * Downloads a submitted image URL and re-hosts it in Supabase Storage
 * (event-images bucket) so the URL is permanent.
 *
 * Body: { url: string, event_id: string }
...
export async function POST(req: NextRequest) {
  ...
  if (!sourceUrl || !eventId) {
    return NextResponse.json({ error: "url and event_id are required" }, { status: 400 })
  }
  ...
  const filename = `events/${eventId}/${user.id}-${Date.now()}.${ext}`
