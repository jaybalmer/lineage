...
//
// PB-009 Phase 4 (substantive, post-launch): override-on-approved, owner appeal,
// auto-surfacing, reporting throttle. Memory: project_pb009_phase4_strategy.md.
// This route's behavior is stable; the strategy memory is the canonical scope reference.
export async function POST(req: NextRequest) {
  ...
  if (typeof parsed.claim_id === "string" && parsed.claim_id) {
    ...
    if (failed > 0) {
      console.error(`[tag-event] claim ${parsed.claim_id}: ${paired} paired, ${failed} failed`)
    }
  }
