...
export async function PATCH(req: NextRequest) {
  ...
  const body = (await req.json()) as {
    ...
    boards_banner_url?: string | null
  }

  const { id, hero_image_url, avatar_url, landing_banner_url, boards_banner_url } = body
  if (!id) {
  ...
  if (boards_banner_url !== undefined) patch.boards_banner_url = boards_banner_url
