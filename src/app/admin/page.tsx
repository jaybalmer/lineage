    ...
    <div className="mb-4 border border-blue-800/40 rounded-xl p-4 bg-blue-950/10">
      ...
      <div className="flex gap-2 items-center">
        <button
          ...
          disabled={!text.trim()}
          className="px-4 py-1.5 bg-[#1C1917] text-white text-xs rounded-lg hover:bg-[#292524] disabled:opacity-40 transition-colors font-medium"
        >
          ...
          <tbody>
                  ...
                  <>
                    ...
                    <td className="py-1 px-2">
                      <div className="flex gap-1">
                        <button onClick={saveEdit} className="px-2 py-1 bg-[#1C1917] text-white text-xs rounded hover:bg-[#292524] transition-colors">Save</button>
            ...
            <tr className="border-t-2 border-blue-900/40 bg-surface/60">
              ...
              <td className="px-2 py-1.5">
                <button onClick={addRow} disabled={!newRow.name.trim()} className="px-2 py-1 bg-[#1C1917] text-white text-xs rounded hover:bg-[#292524] disabled:opacity-30 transition-colors">+ Add</button>
              </td>
            </tr>
          </tbody>
        </table>
            ...
            <tr className="border-t-2 border-blue-900/40 bg-surface/60">
              ...
              <td className="px-2 py-1.5">
                <button
                  ...
                  className="px-2 py-1 bg-[#1C1917] text-white text-xs rounded hover:bg-[#292524] disabled:opacity-30 transition-colors"
                >
                  + Add
                </button>
              </td>
        ...
        <table className="w-full">
          ...
          <tbody>
            ...
            <tr className="border-t-2 border-blue-900/40 bg-surface/60">
              ...
              <td className="px-2 py-1.5">
                <button onClick={addRow} disabled={!newRow.name.trim()} className="px-2 py-1 bg-[#1C1917] text-white text-xs rounded hover:bg-[#292524] disabled:opacity-30 transition-colors">
                  + Add
                </button>
              </td>
            </tr>
                      ...
                      <td className="py-1 px-2">
                        <div className="flex gap-1">
                          <button onClick={() => save(m.id)} disabled={saving}
                            className="px-2 py-1 bg-[#1C1917] text-white text-xs rounded hover:bg-[#292524] disabled:opacity-50 transition-colors">
                            {saving ? "…" : "Save"}
                          </button>
...
export default function AdminPage() {
  ...
  // The server layout (src/app/admin/layout.tsx) is the authoritative gate —
  // only editors are ever served this page. The checks below are a fail-closed
  // client fallback: never render the editor until auth resolves, and never to
  // a non-editor. is_editor OR founding mirrors requireEditorPage().
  ...
  // Fail closed: hold the editor back until the server-validated auth check
  // resolves, so a non-editor never briefly sees the UI.
  if (!authReady) {
    return (
  ...
  // Belt-and-suspenders behind the server layout gate. No password path — editor
  // access is granted by is_editor / founding membership, not a shared secret.
  if (!isEditor) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
