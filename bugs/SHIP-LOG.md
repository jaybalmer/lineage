...
- status: merged
- tsc: clean
- reconcile note (September 8, 2026 morning digest): superseded. All four ids shipped July 20 as PR #163 (`c3a45aa`) and are recorded in the `bug-triage.md` Shipped section; this and the July 11 twin are duplicate unattended-run stubs that never merged as their own PRs. Flipped to `merged` against #163 so the pending list reflects reality.
...
- status: merged
- tsc: clean
- reconcile note (September 8, 2026 morning digest): duplicate of the July 10 stub above; all four ids shipped July 20 as PR #163 (`c3a45aa`). Flipped to `merged` against #163.
...
- status: merged
- tsc: clean
- reconcile note (September 8, 2026 morning digest): this entry sat `pending` while the work itself shipped. The unattended run never produced a commit on this branch; the diff was carried uncommitted in the working tree for eight days, then committed by hand and merged as PR #226 (`7dff426`, September 6). Flipped to `merged` against #226.
...
- commit: 56ab6df
- ids: none
- scope: snowboard-catalog-v0.3
- status: merged
- tsc: n/a
- reconcile note (September 8, 2026 morning digest): auto-stub written by the SessionEnd hook, never expanded. The catalog v0.3 work it names reached prod across PRs #219 to #222 on September 4 and 5 (research artifacts and export tooling, the RetroSnow export refresh to 3,222 boards / 133 brands, the Algorythm spelling fix, and the Ride consolidation), taking the live site to 3,180 boards / 131 brands. Matched on scope and branch name, not on a single PR number, so treat the PR field as approximate; flipped to `merged` to clear a stale pending. Open v0.4 work (41 research gaps, 63 empty museum brands, the Arbor Issuu archive) is unaffected.
...
## 2026-09-04 - catalog v0.3 research artifacts + export tooling (chore)
- type: chore
- pr: #219 (squash sha cd14801)
- branch: claude/catalog-v0.3-artifacts
- ids: none
- scope: catalog-v0.3-artifacts
- migration: none
- status: merged
- tsc: n/a (no src changes; data/scripts/docs only)

Cherry-picked the additive v0.3 catalog research artifacts onto a fresh branch off main (v0.3 branch itself is unmergeable: 13 shared files, main wins). Refreshed existing-export-supabase.csv against production via scripts/export-catalog-tables.mjs (3,180 boards, fixing the #218 catalog-refresh skill whose script was never merged). Added the export script + runbook, v0.3/ + raw/ provenance, 5 sendit/catalog review CSVs, 4 new review CSVs; pointed reconcile-catalog.py at v0.3. Local commit cacb373. Push + PR held pending Jay per the task file's explicit instruction.

## 2026-09-04 - catalog data cleanups + RetroSnow additions (chore)
- type: chore
- pr: #220 (squash sha 2210921)
- branch: chore/catalog-export-refresh-post-retrosnow
- ids: none
- scope: catalog-retrosnow-additions
- migration: none (production data writes applied directly via Supabase MCP, GATED, approved by Jay)
- status: merged
- tsc: n/a (no src changes; data-file export refresh only)

Applied the five catalog follow-ups from the v0.3 merge task to production, then committed the refreshed export (PR #220). DB writes: 3 brand-country fixes (Moonchild->Slovenia, Nobaday->China, Verdad->Belgium), 1 model typo (Lib Tech Douhboy Shredder->Doughboy Shredder), 42 RetroSnow board additions (verified), 4 new brand orgs (H-Street, Haz-Mat, Luxury, Shift; Shorty's already existed), 16 confidence upgrades to verified on matched historic boards (capped model_year<=2000; Sims ATV Pro 2024 outlier left as likely). Boards 3180->3222, brands 129->133. Held for a human call: Ride Algorhythm(2010) vs Ride Snowboards Algorythm(2020) spelling + brand-name split; 2 FUZZY RetroSnow rows (Burton Jeff Brushie DJ, K2 Dan Donnolly XTC); the 63 Michigan-museum zero-model brands (research vein).

## 2026-09-04 - catalog: fix Ride Algorythm spelling (chore)
- type: chore
- pr: #221
- branch: chore/catalog-fix-ride-algorythm
- ids: none
- scope: catalog-ride-algorythm-spelling
- migration: none (single GATED UPDATE to boards applied via Supabase MCP, approved by Jay)
- status: merged
- tsc: n/a (data-file export refresh only)

Fixed the misspelled Ride Algorythm (2010) row (was "Algorhythm") in production and refreshed the committed export. Still open: the Ride vs Ride Snowboards brand-name split for the same model line (needs a canonical-name decision).

## 2026-09-04 - catalog: Ride consolidation + 2 FUZZY resolutions (chore)
- type: chore
- pr: #222
- branch: chore/catalog-ride-consolidate-fuzzy
- ids: none
- scope: catalog-ride-consolidate-fuzzy
- migration: none (GATED board UPDATE/INSERT via Supabase MCP, approved by Jay)
- status: merged
- tsc: n/a (data-file export refresh only)

Consolidated 8 free-text "Ride" boards into canonical "Ride Snowboards" (org o11; no org merge, no collisions). Added Burton "Jeff Brushie DJ" 1994 as a distinct board (3rd-year pro model per RetroSnow, not a dup of "Jeff Brushie 53"). Resolved the K2 "Dan Donnolly Xtc" FUZZY as a confirmed match of existing "Dan Donnelly XTC" 1988 (source attached, no new row). Boards 3222->3223. Closes all v0.3 catalog follow-ups except the 63 zero-model Michigan-museum brands (a research vein).

## 2026-09-04 - BUG-174: public Stack on by default for every member (bugfix)
- type: bugfix
- pr: #208
- branch: claude/public-stack-default-gating-6tzvvg
- ids: BUG-174
- scope: public-stack-default-on
- migration: 20260820000001_bug174_public_stack_default_on.sql (GATED: column default flip plus a backfill UPDATE on pre-existing profiles rows; pre-approved in the brief, applied separately from the merge)
- status: merged
- tsc: clean

Reconciled by the September 5 daily triage, not written in-session. The branch was cut August 20 at 19:38 UTC and merged September 4 at 10:23 PDT, fifteen days, which makes it the longest-stranded finished build in the repo so far. Built from bugs/2026-08-20-public-stack-default-on.md on the four decisions Jay resolved live on August 20. Shipped: starter derivation in readPublicStack (3 most recent public timeline items when a member has not curated), ensureProfile minting a public_slug so every member has a /t/[slug] address, and the member-facing Stack/Timeline pair renamed to Mini/Full (copy-only, internal identifiers keep stack). Owed at review: a signed-out look at a member who has never opened /me/public-view, and a spot check that archived and private profiles were excluded by the backfill.

## 2026-09-05 - Catalog provenance layer Phase 1: "Documented in" source citations (feature)
- type: feature
- pr: #223
- branch: feature/board-sources-provenance
- ids: none
- scope: board-sources-provenance
- migration: 20260905000001_board_sources.sql (SAFE: additive board_sources table + indexes + public-read RLS on a new table; applied via Supabase MCP in-session and verified)
- status: merged
- tsc: clean

Turns the Issuu extraction campaign (15 documents, 1,095 confirmed board sightings across Blue Tomato / Method / Nitro) into a "Documented in" source layer on board pages. New board_sources table (kind existence | image), public read, service-role writes. scripts/import-board-sources.mjs resolves each confirmed sighting to a live boards.id by normalised brand+model (Ride -> "Ride Snowboards" alias per #222) and writes one existence row per documented year; 911/1,059 resolved, the 148 unresolved (models not yet in the live catalog - Salomon/Elan/Rome gaps) logged to issuu-provenance-unresolved.csv and HELD for the normal catalog-merge review, not auto-created. BoardSources component lists citations as DISCRETE documented years (never a continuous range) each deep-linking to the Issuu catalogue - gaps are unsourced years, not absence. No third-party images re-hosted; cite-and-link only. Verified in prod (911 rows) and rendered on /snowboarding/boards/b1 (Burton Custom: 10 discrete years with real gaps). Spec: data/catalog/review/provenance-layer-spec.md. Follow-ups: Phase 2 image-source attribution; the held unresolved brands feed the catalog merge; Arbor 09/10 confirmations (different CSV schema) not yet imported.

## 2026-09-05 - Catalog provenance layer Phase 2: "View in catalogue" image link (feature)
- type: feature
- pr: #224
- branch: feature/board-sources-image-link
- ids: none
- scope: board-sources-provenance
- migration: No migration this session (UI-only; reuses the Phase 1 board_sources table)
- status: merged
- tsc: clean

Adds a single compact "In the {year} catalogue" pointer under the board image (and under the brand-logo placeholder), linking to the earliest documented sighting, so the image ties to where that year's graphic lives. Refactor: extracted useBoardSources hook so the image link and the "Documented in" section share one fetch instead of two; BoardSources now takes an optional sources prop; new BoardImageCatalogueLink. No third-party image re-hosted; cite-and-link only. Verified on /snowboarding/boards/b1. Follow-on to PR #223.

## 2026-09-05 - Track 1: merge Issuu-mined additions into the live catalog (feature)
- type: feature
- pr: #225
- branch: feature/catalog-merge-issuu-additions
- ids: none
- scope: catalog-merge-issuu-additions
- migration: No DDL migration. Additive DATA writes to existing tables via the committed scripts (like prior catalog merges): 20 brand orgs + 441 boards inserted, provenance re-imported to 1,590 board_sources rows. Additive only (no DELETE/UPDATE, nothing near profiles/Stripe); reversible via added_by IS NULL + sources.
- status: merged
- tsc: clean

Turned the Issuu extraction into real catalog rows. scripts/merge-catalog-additions.mjs reads issuu-merge-plan.csv and creates a brand org per new brand + one board per model (Send It shape: added_by null, community_status unverified, year_basis earliest_sourced, confidence likely, model_id slug, sources=Issuu url); idempotent; orgs created only for the new-brand bucket (existing brands already have boards and may carry a differently-named org, so creating one would duplicate it - caught in dry-run). Light dedup collapses exact spelling dups (G Twin/G-Twin), keeps pro/graphic variants distinct. import-board-sources.mjs extended to cite review-candidate sightings that now resolve, and to skip brand-identity-flagged rows (Artec held). Applied to prod: +20 brand orgs, +441 boards (112 new-brand + 329 existing-brand models), catalog 3,223->3,664; provenance now resolves ALL sightings (1,590 citations, 0 unresolved). Follow-ups: Artec identity still held; year_basis is earliest_sourced not introduced; the 63 zero-model Michigan-museum brands remain a separate research vein.

## 2026-09-06 - Unverified badge consistency (bug)
- type: bug
- pr: #226
- branch: fix/unverified-badge-consistency
- ids: BUG-177, BUG-178
- scope: unverified-badge-consistency
- migration: none
- status: merged
- tsc: clean

One shared UnverifiedBadge replaces the three forked treatments of the catalog-status chip (filled pill, five hand-rolled outline copies, and the /claim/[token] palette). Settled on the unfilled amber-600 outline chip because it reads on both the dark app background and inside a .postcard (forced light even in dark mode, repo gotcha 7), which is where the old dark palette was wrong and what the reporter saw. Dropped the glyph. BUG-178: badge now names its entity on claim cards ("unverified place/brand/board/event") so it no longer reads as a verdict on the rider's claim sitting beside ConfidenceBadge; catalog list pages keep the bare "unverified". Board cover overlay keeps its absolute positioning + scrim via className passthrough; add-claim-modal inline pill swept too. Out of scope (D4): the confidence-based "unverified" renders on brands/[slug]/page.tsx (different concept). Presentational only, no migration.

## 2026-09-06 - BUG-179 owner profile claim-flip: diagnosed + ghost cleanup (bug)
- type: bugfix
- pr: none (no code change)
- branch: none
- ids: BUG-179
- migration: none (GATED data cleanup applied, not a schema migration)
- gated-sql: DELETE 1 people row 315a92ff (empty duplicate "Jay Balmer" ghost), approved by Jay, applied + verified 0 remaining
- status: merged
- tsc: n/a (no code change)

BUG-179 was NOT the hydration race the brief hypothesised. Diagnosed as section 4's third possibility (identity mismatch): the reported URL was a duplicate empty unclaimed "Jay Balmer" ghost (315a92ff, 0 claims/0 stories, created by a new member R5 2.5 min after signup), not the owner's own timeline (0394914d, intact). The signed-in "This is me" CTA was correct-by-rules for a genuine ghost, so D1/D2/D3 do not apply. Resolved by deleting the contentless ghost (GATED, approved). Root cause re-triaged for a design pass: all 18 members have zero people rows, so person search backed by the people table cannot match an existing member and users create duplicate nodes. A second live instance (member "Todd Bowman" duplicate node b7bccfdc, 2 claims + 2 story tags, created by Jay Jun 18) is a MERGE case held for the pass. Routing question (how Create A Story reached the ghost) needs replay S-49. See bugs/2026-09-06-duplicate-person-creation-followup.md.

## 2026-09-06 - Duplicate-person prevention Phase 1: gate rider pickers on catalogLoaded (feature)
- type: feature
- pr: #227
- branch: fix/duplicate-person-prevention-phase1
- ids: none
- scope: duplicate-person-prevention-phase1
- migration: none
- status: merged
- tsc: clean

Phase 1 of features/duplicate-person-prevention-brief.md, the client-only fix for the BUG-179 mechanism. SearchPicker gained a loading prop: while catalogLoaded is false it shows a muted non-interactive "Loading riders..." row and suppresses the "+ Add a rider" button and the "None yet" text, so a first-session user with a cold catalog can no longer be funnelled into creating a duplicate node out of an empty list. Wired loading={!catalogLoaded} at the rider/person pickers in add-story-modal, add-connections-popover, add-person-connections-popover, and mention-editor-modal; the event page inline rider search (not SearchPicker) got the equivalent gate plus a handleCreateRider guard. Confirmed the other §1.1 surfaces (add-claim-modal, /people directory, nav, stack-curate) expose no create-new-person affordance, so nothing to gate there. Phases 2-4 (person_search view, dedupe warning, merge_person_into, Todd merge) intentionally out of scope. Verified: tsc clean, dev server compiles, /people and /feed render error-free with members present in the catalog. Merge left to Jay (brief is HUMAN-RUN/ATTENDED); status pending.

## 2026-09-06 - Duplicate-person prevention Phase 2: near-match warning + person_search view (feature)
- type: feature
- pr: #228
- branch: feat/duplicate-person-prevention-phase2
- ids: none
- scope: duplicate-person-prevention-phase2
- migration: applied create_person_search_view (SAFE additive CREATE VIEW, applied to prod via Supabase MCP + verified: 102 people + 15 profiles, todd bowman -> 2)
- status: merged
- tsc: clean

Phase 2 of features/duplicate-person-prevention-brief.md. AddEntityModal person branch now warns before minting a rider: matches catalog.people in nameToSlug space (exact then substring both ways), members (node_status claimed) ranked above ghosts, up to 3 hits, one-click "Use this one" (onAdded existing id) plus muted "Add anyway"; primary Add disabled while a namesake shows but creation never blocked (decision 9.1). Event page inline create routed through AddEntityModal so the same warning runs there (removed bespoke handleCreateRider; kept the Phase 1 catalogLoaded dropdown gate). person_search view applied to prod: canonical profiles+people union, id cast to text, people branch NULLs avatar_url (no such column), security_invoker true; exists so a future server-side rider lookup cannot reintroduce the missing-member class, client dedupe still reads catalog.people. Warning UI verified by inspection + live view data (create is auth-gated in preview). Phase 3 (merge_person_into, GATED) and Phase 4 (Todd merge) next.

## 2026-09-06 - Duplicate-person prevention Phase 3: merge_person_into (feature)
- type: feature
- pr: #229
- branch: feat/duplicate-person-prevention-phase3
- ids: none
- scope: duplicate-person-prevention-phase3
- migration: applied 20260906000001_merge_person_into.sql (GATED, approved by Jay, applied via Supabase MCP + verified: both functions live, merge_log.claim_request_id nullable + note col; Todd dry-run returned the expected 9-ref shape then rolled back clean)
- status: merged
- tsc: clean

Phase 3 migration written and up for review; NOT applied. Adds merge_person_into (generalized fold-in that works when the canonical is a profiles member, which the original merge_person cannot do), repoints every person-referencing column incl the 6 drifted since 2026-05-11, real dry-run via DRYRN savepoint, person_slug_aliases + merge_log snapshot, ghost delete; merge_person delegates its Path B to it. merge_log claim_request_id relaxed nullable + note column added. Verified read-only against prod (types, PKs, FKs, dedup keys; person_invite_notifications is the only CASCADE FK to people(id), handled). Held at two GATED gates awaiting Jay: apply the migration, then run the Todd Bowman merge (b7bccfdc -> profiles d644c90b) dry-run (baseline: tag_events x4, claims.subject_id x2, story_riders x2, claim_requests.node_id x1) then commit. Route + admin UI + promote-ghost/invite-claim refold follow once the function is applied. Phase 4 (Todd merge) blocked on this.

## 2026-09-06 - Duplicate-person prevention Phase 4: Todd Bowman ghost merged into member (data op)
- type: chore
- pr: none (GATED data operation via merge_person_into)
- branch: none
- ids: BUG-179 (follow-up 1, second live instance)
- scope: duplicate-person-prevention-phase4
- migration: none (data op; used the Phase 3 merge_person_into RPC)
- gated-sql: SELECT merge_person_into('b7bccfdc-...ghost','d644c90b-...Todd profile','profiles',<jay>,false,'BUG-179 follow-up 1') - dry-run reviewed + approved by Jay, then committed + verified
- status: merged
- tsc: n/a (no code change)

Folded the duplicate unclaimed "Todd Bowman" ghost node (b7bccfdc, created by Jay 2026-06-18, 2 claims + 2 story tags + 4 paired tag_events + 1 open claim_request) into Todd's real member profile (d644c90b) using the new merge_person_into RPC with canonical_kind=profiles. Dry-run first (matched the 9-ref pre-flight baseline exactly, rolled back clean), then committed on Jay's go. Verified: ghost row gone, profiles.merged_from_id set, claims moved (Todd 25 -> 27), tag_events/story_riders/claim_requests all repointed, merge_log row written, person_slug_aliases (ghost id + todd_bowman slug -> Todd) so old /people URLs 308. person_search now returns one Todd Bowman. Closes the second live duplicate flagged in bugs/2026-09-06-duplicate-person-creation-followup.md. Remaining: rest of Phase 3 (/api/admin/merge-person route + /admin/claims panel + promote-ghost/invite-claim refold for Defect 2) and the decision 9.2 RUN-LOG reference sweep across previously promoted ghosts.

## 2026-09-06 - Ghost-promotion refold onto merge_person_into (Defect 2) (bug)
- type: bug
- pr: #230
- branch: fix/promote-ghost-full-repoint
- ids: BUG-179 (follow-up 1, Defect 2)
- scope: promote-ghost-full-repoint
- migration: none (uses the merge_person_into RPC already applied in #229)
- status: merged
- tsc: clean

The three ghost-claim completion paths (public claim-complete, admin-invite-complete, invite/claim) repointed only 4 of ~20 person columns then deleted the ghost, dropping tag_events (visibility layer), community/event junctions, mentions, stack entries, catalog added_by, riding_days, claim_requests.node_id, and cascade-deleting person_invite_notifications; the plain story_riders repoint could also throw on an already-tagged story. Routed all three through merge_person_into (canonical_kind=profiles): one schema-current repoint list + composite-key dedup + person_slug_aliases + merge_log + ghost delete, transactional. promote-ghost.ts keeps the identity-restore the RPC does not do and never throws (failure -> claimed:false, not 500); invite/claim drops its duplicated inline block and calls the same helper. tsc clean, no migration. Left pending: auth-adjacent + data-deletion path, so held for Jay's merge review rather than self-merged.

## 2026-09-06 - Admin duplicate-person merge panel (feature)
- type: feature
- pr: #231
- branch: feat/admin-merge-panel
- ids: none
- scope: admin-merge-panel
- migration: none (uses merge_person_into RPC live since #229)
- status: merged
- tsc: clean

Completes Phase 3 of features/duplicate-person-prevention-brief.md: the editor front end for merge_person_into so future duplicates are merged in-app, not by hand SQL. POST /api/admin/merge-person (requireEditor, dry_run defaults true, commit requires confirm_name == ghost display_name, refuses self-merge + claimed ghost, returns RPC jsonb) + GET /api/admin/person-search (canonical picker over person_search view, members first) + AdminMergePanel (search ghost via invite-node + canonical via person-search, preview dry-run counts, typed-name confirm to commit) mounted on /admin/claims next to AdminInvitePanel. No migration. tsc clean; anon gets 401/307 on all three surfaces; merge RPC proven by the Todd merge. Held pending: commit path can delete a ghost (data-deletion), so awaiting Jay's merge review.

## 2026-09-06 - Defect 2 orphan-reference repair (data op)
- type: chore
- pr: none (GATED data op)
- branch: none
- ids: BUG-179 (follow-up 1, Defect 2 past damage)
- scope: promote-ghost-orphan-repair
- migration: none (targeted data repair)
- gated-sql: one transaction, dry-checked then approved by Jay: repoint 15 refs + delete 1 colliding row across tag_events/invites/mentions/public_stack_entries/event_people, ghost->canonical (208a8e52->Jay 0394914d, adde535d->John 07e6edac)
- status: merged
- tsc: n/a (no code change)

Cleaned the 16 dangling references the old 4-of-20 promote-ghost path left pointing at 2 now-deleted ghost ids (found by the §9.2 sweep). Repointed tag_events x9 + invites.person_id x3 + mentions x2 + public_stack_entries x1 onto the canonical member profiles; deleted the 1 event_people row that duplicated the canonical on its composite PK. Re-ran the sweep: 0 orphans remaining, references confirmed on the canonicals. Closes the Defect 2 past-damage question; no separate repair brief needed. Recorded in bugs/RUN-LOG.md.

## 2026-09-06 - Feed same-day claim grouping (feature)
- type: feature
- pr: #232
- branch: feat/feed-claim-grouping
- ids: BUG-076
- scope: feed-claim-grouping
- migration: none
- status: merged
- tsc: clean

Closes BUG-076 (three reports across eleven weeks): the feed rendered one card per claim, so a member logging a session of many resorts or boards flooded it and buried the stories. A member's run of 3+ claims added on the same day now folds into one header-only card ("Dano added 6 places and 1 connection to their timeline"), expandable to the individual PostCards with companion chips intact. New src/lib/feed-grouping.ts (grouping + by-type summary) and src/components/feed/claim-group-card.tsx (modeled on MentionEpisodeGroup); feed/page.tsx runs it after the existing companion fold. Decisions confirmed live with Jay: key person+day, threshold 3, summarize by type, header-only collapsed, and grouping only in Recently-added sort (Date-happened keeps event chronology with individual cards). Verified signed-out against prod data in the browser: two groups rendered, 2-item runs stayed individual, expand and the sort-mode fallback both correct, no console errors.

## 2026-09-06 - PostHog history-change pageviews + reset on sign out (feature)
- type: feature
- pr: #233
- branch: chore/posthog-history-pageviews
- ids: none
- scope: posthog-history-pageviews
- migration: none
- status: merged
- tsc: clean

Step 0 carve-out of features/funnel-event-completion-brief.md T14, shipped alone ahead of FNRad episode-1 traffic. instrumentation-client.ts: capture_pageview true -> "history_change" so App Router client-side navigations are counted (the boolean form only captured hard loads, overstating every page-level funnel denominator). avatar-dropdown.tsx handleSignOut: posthog.reset() (try/catch, runs after signOut, before redirect) so the next anonymous session is not attributed to the signed-out user. No migration, no new dep (posthog-js already installed); capture_pageview: boolean | 'history_change' is a declared @posthog/types field so tsc is clean. Self-merged: SAFE per risk gate (analytics-only, cannot affect the auth flow it sits beside). Handoff lead is the second PR of this session.

## 2026-09-06 - Signup intent handoff (feature)
- type: feature
- pr: #234
- branch: feat/signup-intent-handoff
- ids: none
- scope: signup-intent-handoff
- migration: none
- status: merged
- tsc: clean

Implements features/signup-intent-handoff-brief.md T1-T6, the FNRad episode-1 critical path: carry a small add-story "intent" through the whole signup chain inside returnTo (validated by safeReturnTo at every hop) and replay it once on arrival so a listener lands back on the guest's page with the composer pre-tagged, instead of on their own empty profile. New src/lib/intent.ts (closed action allowlist add-story, encoder, uuid-validated reader, stripper). T2 SignInPrompt path+search + signed-out person-page CTA. T3 signin "Create your timeline" carries returnTo + OAuth error bounces keep it. T4 proxy stamp + OnboardingFlow read/forward + SaveStep OAuth/magic-link/OTP + pending_onboarding cross-device stash + /auth/complete re-validated read. T5 subject-matches-page replay that clears URL+history. T6 named header + pinned tag chip + first-story timeline default. No migration, no schema, no new endpoint, no new analytics. Absorbs R4 in full + one line of R7 from funnel-drop-off-repairs-brief.md. tsc clean; lint 0 new errors/warnings. Local smoke: signin returnTo link + onboarding read verified; A1-A3 (incl. cross-device) and the signed-out CTA are deploy-time (anon mock-person pages 404 in dev, pre-existing). status pending: session touched auth flows, so held for Jay's merge review per the Ship-sequence exception. Deploy-time F18 check owed: episode-1 guest must have a uuid people.id and require_tag_approval not true.

## 2026-09-06 - FNRad listener landing (feature)
- type: feature
- pr: #235
...
Implements features/fnrad-listener-landing-brief.md, the last code piece before FNRad episode 1: a public listener hub at /fnrad, a weekly guest-story challenge keyed off one config constant, and the episode-surface passes that turn the chromeless episode pages into entry points. New src/lib/fnrad.ts (FNRAD_SHOW_SLUG confirmed "fnrad_podcast", FNRAD_CHALLENGE, addStoryAboutHref), src/lib/mention-links.ts (shared mentionHref), src/components/fnrad/challenge-card.tsx (shared hub+episode card), src/app/fnrad/page.tsx (force-dark server hub that degrades to explainer+CTA+challenge+equity when the show slug fails to resolve). readShowHub()/readChallengeGuest() + PublicMention.href + PublicEpisodePayload.challengeGuest in public-timeline-read.ts. Episode page (public-episode-view): listener strip, challenge card, linked mention names (T4), linked guest chips (T8), footer CTA. In-app episode logged-out CTA now carries the guest/episode instead of "/" (T9). Sparse-person empty state renders its actions for signed-out visitors and drops the false "mentioned them" copy (T7a). Challenge spotlight is the same person on every surface (from FNRAD_CHALLENGE.guestId, never an episode's own guests), so the hub and episode pages cannot disagree. No migration, no API route change, no auth/onboarding change; addStoryAboutHref routes everyone through the guest's page, which PR #234 already makes correct per auth state. Section 0 pre-flights run 2026-09-06 against Ingemar Backman (people.id fbf34ae7-f056-4457-ad90-3be67adac0f1): PF1 permissive PASS, PF2 uuid PASS, PF3 board already spelled correctly RESOLVED; guestId ships pointed at that verified id. Local smoke passed A1-A5, A7, A8a/A8b/A8c, A10, A11, A12/A12b/A12c/A12e, A13, A15, A16. Self-merged: SAFE per risk gate (no migration, no API, no payments/auth/data-deletion path). Deploy-time acceptance owed: A9 full signed-out listener walk (writes prod data, run against a test-episode guest) and the signed-in half of A12e.

## 2026-09-07 - Feed claim attribution (bug)
- type: bug
- pr: #236
- branch: bugfix/feed-claim-attribution-183-184
- ids: BUG-183, BUG-184
- scope: feed-claim-attribution
- migration: none
- status: merged
- tsc: clean

BUG-183 (P1): the community feed named the claim SUBJECT as the actor, so a non-member ghost got a card reading as if they had added to their own timeline. authorForClaim (feed/page.tsx:209) resolved subject_id while claims.asserted_by sat unread on the payload. Split into subjectForClaim + a new actorForClaim that resolves asserted_by against catalog.people with a miss-tolerant string compare (asserted_by is TEXT, may be a legacy person id or orphaned uuid). Both feed call sites and the group card now name the actor; unresolved actors fall back to "A rider" unlinked, including the grouped card which previously rendered a blank gap. Grouped card copy is now "MEMBER added N to SUBJECT's timeline", collapsing to "their timeline" when actor and subject match. Group key moved from subject_id|day to asserted_by|subject_id|day (feed-grouping.ts), the D2 alternative, so every group is single-subject and that sentence is always unambiguous; claims with no asserter never group. BUG-076 grouping preserved (3+ same-day adds, Recently-added sort only). BUG-184 (P2): a claim whose object no longer resolves rendered the literal "Unknown" in a dead "#" link. PostCard now renders an unlinked type-based fallback ("A board" / "A place") when the object misses, without touching the shared getEntityName; the "#" href becomes undefined and the graphic/title render as plain text. Section 4 diagnostic returned 0 orphaned board claims in prod, so the miss is client-state only (an anonymous viewer resolving real public claims against the mock catalog), not a data bug. Verified live on /snowboarding/feed: grouped card reads "Jay Balmer added 1 place, 2 events and 1 entry to Ingemar Backman's timeline"; same-subject groups collapse to "their timeline"; no console errors. Client render only, no migration/view/write-path change. Self-merged: SAFE per risk gate (no migration, no auth/payments/deletion); merge confirmed by Jay (attended, primary-surface copy). Squash sha 949ac7d.

## 2026-09-07 - Self-tag auto-approve (bug)
- type: bug
- pr: #237
- branch: bugfix/self-tag-auto-approve-180
- ids: BUG-180
- scope: self-tag-auto-approve
- migration: GATED backfill applied (tag_events self-tag pending->approved, 7 rows)
- status: merged
- tsc: clean

BUG-180 (P1, attended): tagging yourself as a rider in a story you authored created a member tag_event that defaulted to pending (since PB-009 Phase 2, commit 7bc79bd) and landed in your own /me/tags inbox. The claims path already skipped self-tags (tag-events.ts:396) but the story-rider path had no guard; PR #229's merge RPC (first ever to repoint tag_events.subject_id) surfaced aged ones at once. Fix: added the self-guard in insertTagEvent, the shared primitive, so every paired-write path gets it in one place (story POST+PATCH, POST /api/stories/[id]/connections per D4, and defensively claims). A self-tag (asserter_id === subject_id) is still written, keeping the story_riders_public invariant (D1 default), but lands approved with decision_by = the subject, mirroring the trust path; asserter_id is null on anonymous public-embed tags so the check never fires there. Verified the two direct insertTagEvent callers are safe (public/tag null asserter; admin scan-results editor/system already approved). Fixed the stale pairStoryRiderTagEvents header comment. Did not touch /api/me/tags, the views, or defaultStatusForSource. GATED backfill: pre-check found 7 pending self-tags (3 story + 4 claim-sourced), widened per D3 alternative to clear all of them. Confirmed live columns are decision_at + decision_by (brief said decided_at, which does not exist); subject_id is text so cast to uuid decision_by. Applied with Jay's approval: UPDATE tag_events SET status='approved', decision_by=subject_id::uuid, decision_at=now() WHERE subject_id=asserter_id::text AND status='pending'. Verified: pending self-tags dropped to 0 (approved-story 6->9, approved-nonstory 0->4; disabled rows untouched). Self-merged after backfill applied+verified; gated step resolved with approval, not on the payments/auth/deletion exception list. Squash sha 7426485.

## 2026-09-07 - Attribution, episode-1 carve-out (feature)
- type: feature
- pr: #238
- branch: feat/funnel-attribution
- ids: none
- scope: funnel-attribution-ep1-carve
- migration: none
- status: merged
- tsc: clean

Episode-1 carve-out (section 11b of features/funnel-attribution-brief.md), the measurement half of the FNRad run so episode 1 produces attributable signups. Strict forward-compatible subset of the full brief: writes the exact localStorage shape the full acquisition build later reads, so nothing is discarded. New src/lib/attribution.ts is the shared first-touch storage contract: parseTouch (utm_* + ref + external document.referrer, tight [a-z0-9_.-] sanitiser capped at 64, query stripped from landing_path, returns null on no signal so an internal click never overwrites last-touch), captureTouch (90-day whole-record expiry, first-touch immutable, last-touch updated, every localStorage access try/catch wrapped), readAttribution/clearAttribution, and attributionProps() emitting flat attr_*-prefixed short strings with nulls omitted and the referrer reduced to HOST only (D8 PII rule). captureTouch() wired into src/instrumentation-client.ts after PostHog init, before hydration, double-guarded so it can never break client instrumentation. attributionProps() stamped onto signup_started (google/facebook/magic_link in save-step.tsx) and signup_succeeded (auth/complete/page.tsx) ONLY; ftue_landed, ftue_completed, the /r/[code] route, the acquisition/ref_codes tables and the /admin/users column all wait for the full build. No migration, no new tables. FNRad surfaces already ship plain ?ref=fnrad params (PR #235). Verified in a dev preview against the real module: A2 capture (first==last, query-stripped landing_path), A3 first-touch immutability, A4 no-signal no-op, A5 90-day expiry replace, and the PII-free prop shape (all attr_-prefixed short strings, term omitted, referrer reduced to podcasts.apple.com host only); tsc clean, no console errors. Merge deferred to Jay: touches two auth-flow files (save-step.tsx, auth/complete/page.tsx) though it only adds event props to trackEvent calls; the brief argues that is not an auth-mechanism change, but the repo Ship sequence lists auth flows as a prompt-Jay exception and the session was attended. Merged with Jay's go-ahead; squash sha 0294b7e.

## 2026-09-07 - Events catalog import (feature)
- type: feature
- pr: #239
- branch: feature/events-catalog-import
- ids: none
- scope: events-catalog-import
- migration: 20260907000001_events_catalog.sql (SAFE/additive, applied to prod this session)
- status: merged
- tsc: clean

Imported the researched snowboard events catalog (data/events/v0.3/) into prod per features/events-catalog-import-brief.md: one additive migration, import tooling, a reconciliation pass, no UI. Migration adds nullable provenance columns to event_series and events plus a new event_results podium table with a select-using(true) RLS policy, indexes, and a NULLS NOT DISTINCT slot-uniqueness constraint; classified SAFE and applied via the Supabase MCP, then verified (9 events cols, 10 series cols, table+policy+3 constraints present). Step 0 schema probe was run directly against prod via MCP (the Cowork session that wrote the brief had no network route), confirming the brief's defaults: events.id and start_date are both text, so slug-as-id and partial-date-as-string held and no --widen was needed. scripts/reconcile-events.py (name+year, four buckets) found 16 catalog editions duplicating existing prod rows; the first pass missed the Baker family because the catalog says "Mt. Baker Legendary Banked Slalom" vs the live "Baker Banked Slalom", fixed by dropping "legendary" in the normaliser, which surfaced all 10 Baker years incl. the 2019 edition on the landing-page timeline. Jay confirmed a clean merge (and NASC-1988 fuzzy = same event): no duplicate rows created, the 33 new Baker + 2 new Westbeach editions fold into existing series es1/Westbeach, and the 65 podium results of the 16 colliding editions re-point onto the existing event rows (so 2019 Baker gained its 17 placings rather than being cloned). Import ran through PostgREST upsert (scripts/upload-events.mjs, service-role key from .env.local, FK order series->events->results) since no psql/DB URL was available and inlining 1.2MB of SQL through MCP was wasteful; scripts/build-events-sql.py emits both SQL and JSON from one merge-aware source. Net prod effect: +119 series, +599 events, +3915 results. Verified against prod: 0 duplicate events, 0 orphaned results, 0 results without a source, 0 podiums on cancelled editions, exactly 1 "2019 Baker" row, es1 now 43 editions, still 1 Baker series; re-running the uploader left counts unchanged (idempotent). No new Supabase security advisory (event_results ships with its policy). Self-merged: additive migration already applied, no payments/auth/deletion paths touched. Squash sha 2e62bb1. Deferred to a later session (out of scope here): member event-tagging UI/model, rider-to-person linking (event_results.person_id stays null), and the v0.4 research gaps in data/events/v0.3/review/.

## 2026-09-07 - Events catalog: drop never-held editions (feature follow-up)
- type: feature
- pr: #240
- branch: fix/events-drop-neverheld-editions
- ids: none
- scope: events-catalog-import
- migration: none (prod data DELETE of 20 imported rows, applied with Jay's approval)
- status: merged
- tsc: clean

Follow-up to #239 after Jay flagged the Mt Baker LBS list had too many editions. The import created a row per scheduled year including postponed/never-held ones. scripts/build-events-sql.py now skips catalog status cancelled/postponed/unconfirmed (overrides brief DECISION 5); emitted new events 599->579. In prod (GATED data-deletion, Jay approved via question) deleted 20 never-held rows (18 cancelled + 2 unconfirmed), each verified 0 podiums/0 stories/0 claims/0 junctions: DELETE FROM public.events WHERE external_ref LIKE '%--%' AND catalog_status IN ('cancelled','unconfirmed'). Baker es1 43->38 held editions ending 2025 (36th); 0 never-held remain catalog-wide; events total 630->610; results unchanged 3915. Reversible via re-import. Squash sha 9ee99be.

## 2026-09-07 - Events: Mt Baker LBS naming + non-run-year cleanup (feature follow-up)
- type: feature
- pr: #241
- branch: fix/lbs-naming-and-count
- ids: none
- scope: events-catalog-import
- migration: none (prod DELETE of 2 rows + UPDATE of 36 names on es1, applied with Jay's approval)
- status: merged
- tsc: clean

Follow-up to #239/#240 on the Mt Baker Legendary Banked Slalom series (es1), both Jay-requested. (1) Renamed every LBS edition to "[YEAR] Mt. Baker Legendary Banked Slalom" (was a mix of catalog edition_labels like "30th ..." and old community names "Baker Banked Slalom 1987"); build-events-sql.py now emits that name for the Baker series so the idempotent upsert won't clobber it. (2) Reconciled 38->36 editions: the extra two were 2005 and 2015, both years the race did not run (2015's 30th ran Feb 2016; 2005 a documented no-run year). Both pre-existing community rows (catalog_status=null, missed by #240's never-held sweep), 0 podiums / 0 deps. Prod (GATED, Jay-approved): DELETE FROM public.events WHERE series_id='es1' AND year IN (2005,2015); UPDATE public.events SET name = year::text || ' Mt. Baker Legendary Banked Slalom' WHERE series_id='es1'. Verified: es1 now 36 editions 1985-2025, all names conforming, 0 non-run years. Squash sha 911be17.

## 2026-09-07 - Events list flatten + continent grouping + locations + podium results (feature)
- type: feature
- pr: #242
- branch: feat/events-list-flatten-locations-results
- ids: none
- scope: events-catalog-import
- migration: none
- tsc: clean
...
Made the imported events catalog navigable/visible (frontend, no DB change). Events index: removed the event-type filter chips + per-card type label/colour (flat list, not divided by type), added a "Group by Decade | Continent" toggle (continent from imported country via new src/lib/continents.ts), and cards now show location (linked Place else imported venue/city/country). Event detail page: header shows imported venue/city/country when no place_id, plus a new read-only Podium section fetching public.event_results grouped by discipline then division (medals, rider name, nationality, score/time). Rider identity confirmed decision: rider_name stays plain text, NO bulk creation of ~1,623 person nodes (import DECISION 3), claim-on-demand later. Plumbing: Event type + new EventResult type in src/types/index.ts; catalog already selects "*" so columns arrive; event_results public-read fetched client-side. Verified in local dev preview against real catalog: continent grouping with full locations, 2025 Mt Baker edition showing 70 placings by division with times, no console errors. Naming ("[YEAR] Mt. Baker Legendary Banked Slalom") already live from #241; the es1 series row itself is still "Baker Banked Slalom" (offered to rename separately). Squash sha cdb0224.

## 2026-09-08 - localStorage quota overflow from persisted userEntities mirror (bug)
- type: bug
- pr: #243
- branch: fix/localstorage-quota-userentities
- ids: BUG-188
- migration: none
- status: merged
- tsc: clean

P0 QuotaExceededError that broke every store write for signed-in members. The persisted zustand store crossed WebKit's hard 5MB per-origin localStorage quota: userEntities is a full mirror of the places/boards/orgs/events catalog tables (measured 4.96MB, tipped over by the Sep 7-8 events import #240-#242), and zustand v5 runs localStorage.setItem synchronously inside set() with no try/catch, so once over quota every store action threw, not just addClaim (where the throw landed before the DB write, so claims silently never saved). Fix, all in src/store/lineage-store.ts: partialize now drops userEntities for signed-in users only (Nav refetches it via loadDbEntities on every mount, so persistence buys nothing; anonymous adds keep persisting it so a mid-onboarding reload survives); version 1 + migrate strips any blob already on disk (partialize governs writes, not reads); and a quota-safe storage wrapper degrades a full quota to a skipped save instead of throwing into React handlers (same guard as use-board-image.ts and seen-celebrations.ts). Owed at review: a real-iPhone WebKit smoke, since Chrome DevTools device mode runs Blink and cannot reproduce the quota. Squash sha f1e8d38.

## 2026-09-08 - Grouped feed card clipped the member name at mobile widths (bug)
- type: bug
- pr: #244
- branch: fix/feed-grouped-card-name-truncation
- ids: BUG-187
- scope: feed-grouped-card-name-truncation
- migration: none
- status: merged
- tsc: clean

P2 layout fix on the community feed. The grouped claim card's header was a non-wrapping flex row with the actor name carrying `truncate` beside the summary sentence; truncate's overflow:hidden dropped the name's min size to zero so all shrink landed on the name, clipping it to "Jay.." at mobile widths (#232 added the truncate, #236 lengthened the sentence to "{subject}'s timeline" and made it visible). Fix in src/components/feed/claim-group-card.tsx: render the header as one wrapping text flow (name inline, sentence inline, explicit space, no inner flex, no truncate on the name), so the name is a word in the sentence and never a separately-shrinkable box. Both actor branches (linked Link and the unresolved "A rider" span) preserved with their weight and hover. story-card.tsx:328 keeps its deliberate BUG-158 truncate and has zero diff. Verified in the Browser pane at 375px: "Jay Balmer added 1 place, 2 events and 1 entry to Ingemar Backman's timeline" shows the full name and wraps, short cards and the "their timeline" variant render correctly, no console errors. Squash sha 1deecab.

## 2026-09-08 - Board detail page credits the contributor (bug)
- type: bug
- pr: #245
- branch: fix/board-page-added-by-credit
- ids: BUG-185
- scope: board-page-added-by-credit
- migration: none
- status: merged
- tsc: clean

P2 display-only fix. The board detail page showed no "Added by" credit though the boards list page and feed both do. boards.added_by is already on the board object the page holds (store fetches boards with select("*")), so no fetch or migration. Added a credit line below the year in src/app/(community)/[community]/boards/[id]/page.tsx: "Added by {name}" as small muted text matching board-parts.tsx, name linked via usePersonHref, contributor resolved against catalog.people so an unresolved/archived added_by renders no line (no "Added by undefined"). D1 default taken: credit shows on every board with a resolvable added_by, verified or not; the list page keeps its isUnverified gate and is unchanged, so the two surfaces deliberately differ now (list-page parity left as a separate call, flagged in the PR). Verified in the Browser pane at 402px on a real member-added board (Storm Snowboards Cyclone 1988, added by Jay Balmer): "Added by Jay Balmer" links to /people/jay_balmer, no overflow; a board with no resolvable added_by shows no line. Squash sha 66cd6fc.

## 2026-09-08 - Funnel drop-off repairs, core (feature)
- type: feature
- pr: #246
...
PR 1 (core tier: R1, R2, R6) of the funnel drop-off repairs brief. Squash sha f0cc660. The three
silent drop-offs a cold FNRad-callout visitor actually hits now speak up. R0 new
src/lib/auth-messages.ts, one home for the auth error copy keyed by the codes
already in the redirect URLs with a generic fallback. R1 reads the error param
on all four silent landings (/onboarding dismissible banner under the thread;
/auth/signin and /auth/forgot-password existing error slots), via
window.location.search so no page is forced dynamic, never rendering the raw
code. R2 sends an already signed-in member off /onboarding to /me/timeline,
authReady-gated so a persisted stale id cannot misfire, holding on the brand-mark
state so the FTUE never paints for a member. R6 the magic-link sent state gains
a resend with a 30s cooldown and a visible failure surface inside the sent state
on both the save step and /auth/signin; the route additionally accepts the
Vercel preview host origin (anchored match) so a preview stops emailing links
into production, and carries a failed onboarding stash out as
{ warning: "stash_failed" } that the client records as one analytics event and
shows nothing for. Verified: tsc clean; A2/A3/A4 error landings and A16 375px
render checked in the Browser pane; no server/console errors. A7/A8/A9 (live
magic-link send + preview-host link) owed to the deploy walk per brief section
13, since they hit prod Supabase/Resend and need the Supabase Redirect URLs
wildcard. status pending: auth-flow adjacency, so surfaced to Jay for the merge
rather than self-merged. No migration.

## 2026-09-08 - Funnel drop-off repairs, second tier (feature)
- type: feature
- pr: #247
...
PR 2 (second tier: R3, R5, R7, R8) of the funnel drop-off repairs brief, on top
of merged PR #246. R4 was absorbed into and shipped with the signup-intent
handoff (PR #234), so it is not rebuilt. R7 signup leads the contribution prompt:
SignInPrompt gets three stacked actions (Create your timeline / I already have an
account / Not now), both links carrying returnTo, Appendix A copy, message prop
override preserved; /auth/signin promotes the signup affordance from a fontSize:10
inline link to a full-width bordered button below the consent line (destination
unchanged, shape only). R3 opt-in resume: a returning visitor with both answers
saved gets a "Pick up where you left off" button on the first step only that jumps
to the save step, completeness captured before the BUG-166 step-0 reset so nobody
is dropped mid-flow. R5 claim dead ends: not-found and expired now offer "Start
your timeline" + "Sign in" instead of a single button to /, the already-claimed
and valid-invite secondaries repointed from /onboarding to /auth/signin (invite
token already in localStorage so binding survives), em dash removed. R8 password
sign-in defaults to /me/timeline like every other method. Verified in the Browser
pane: tsc clean; A10 prompt returnTo, R7 promoted button, A12 resume-to-save, A13
claim not-found two-action, A16 375px all checked; no server errors. A11/A14/A15
(live auth round-trip + real invite token) owed to the deploy walk. status pending:
auth-flow adjacency, so surfaced to Jay for the merge rather than self-merged. No
migration.

## 2026-09-09 - Board "Add to timeline" created duplicate owned_board claims (bug)
- type: bugfix
- pr: #248
- branch: fix/board-add-duplicate-claims
- ids: BUG-190
- scope: board-add-duplicate-claims
- migration: none (gated dedupe + partial unique index DEFERRED to Jay, section 6 of the brief)
- status: merged
- tsc: clean

P1 data bug. A real member on mobile Safari tapped a board's "Add to timeline" rapidly and created 10 duplicate owned_board claims for the same board. BoardActionsMenu.handleAdd had no in-flight guard, so each tap fired its own addClaim -> POST /api/claims; the server's owned_board backstop (route.ts:136-158) is a check-then-update, not atomic, so N concurrent requests each saw zero rows and each inserted. The button only hides once the card re-renders to claimed, which is async, so same-frame taps all got through. Fix: synchronous submittingRef guard plus an already-`existing` short-circuit at the top of handleAdd, reset on each popover open, submitting state disabling the button after the first tap. Same guard applied to QuickClaimPopover (shared gap, short-circuit on alreadyClaimed). Client only, no migration, toast unchanged, no blocking dialog (Decision 2). The DB dedupe of the existing Winterstick duplicates and the partial unique index on claims (subject_id, object_id) WHERE predicate='owned_board' delete/alter existing rows, so they are a GATED follow-up left for Jay. tsc clean; race guard verified by reasoning (ref set synchronously before addClaim, re-entry returns early), single-tap add and claimed badges unchanged. Squash sha f34e355.

## 2026-09-09 - owned_board atomic dedup: unique index + conflict upsert (bug follow-up)
- type: bugfix
- pr: #249
- branch: fix/claims-owned-board-unique-index
- ids: BUG-190
- scope: claims-owned-board-unique-index
- migration: applied - partial unique index claims_owned_board_uniq on claims (subject_id, object_id) WHERE predicate='owned_board' AND parent_claim_id IS NULL, via CREATE INDEX CONCURRENTLY (execute_sql, not apply_migration; CONCURRENTLY cannot run in a txn)
- status: merged
- tsc: clean

Follow-up to PR #248's client guard, closing the residual race the guard cannot cover (two devices/tabs, retry storms, any future insert path). The section-6 dedupe DELETE was a confirmed no-op: prod had 57 owned_board rows = 57 distinct subject+board pairs (the reporter's 10 Winterstick duplicates had already been removed by hand), so the partial unique index built cleanly with no destructive step. Applied via execute_sql because CREATE INDEX CONCURRENTLY cannot run inside a transaction; verified indisunique=true, indisvalid=true. Code: POST /api/claims now catches the 23505 owned_board conflict and resolves it as the existing check-then-update backstop would (look up the row, widen board_relationship/start_date, return ok+updated id) instead of a client-rolled-back error; non-owned_board conflicts keep the 409. The originally-GATED destructive dedupe is therefore fully retired, not deferred. Squash sha 0ebff79.

## 2026-09-09 - Presenting partner surfacing (feature)
- type: feature
- pr: #250
...
Parts (a) and (c) of the September 7 [Linestry Idea] FNRad request, built as a generic presenting-partner mechanism driven by the existing orgs.curation_tier = 'founding' (no migration, no new columns, no admin work). New src/lib/partners.ts holds the shared resolution helpers (isPresentingPartner / presentingPartners / primaryPresentingPartner / partnerEyebrow) so the two surfaces cannot disagree; presentingPartners() dedupes by id so the live BUG-189 catalog double-fetch can never render a partner twice. Brands index: a "Presenting Partner(s)" group pinned above every sort and filter, derived from allOrgs (not brandOrgs) so a media-type partner is not invisible (F3), with featured orgs lifted out of their home buckets so each renders once (D3); the founding card shows partner_label when set else "Curated" (T2.4). Landing page: one presenting-partner card below the snowboarding focus card, gated on catalogLoaded so it cannot shift the primary CTA (BUG-017/D8), logo on a white tile to survive the force-dark scope (F10). Step 0 found Westbeach (o15) already at 'founding' in prod (Brand Page Redesign smoke leftover); per Jay's call it was reset to 'curated' (execute_sql UPDATE, GATED but explicitly approved) BEFORE merge so both surfaces stay empty until a real partner is flipped at /admin/brand/[id]. Verified locally against the real prod catalog via SPA nav: PP section renders first, survives Category/A-Z sorts, partner lifted (once, absent from flat list and category group), search hides/restores it, landing card links to the brand page and is legible in dark scope, 0px horizontal overflow at 375px on both surfaces. lint clean except the pre-existing unused-totalBrands warning. Squash sha 1f844aa.

## 2026-09-09 - Ops tree hygiene: stop the ops loop blocking the code pipeline (feature)
- type: feature
- pr: #251
- branch: ops/tree-hygiene-brief
- ids: none
- scope: ops-tree-hygiene
- migration: none
- status: merged
- tsc: clean

Repairs the reason the 05:00 unattended run aborted nine of the last ten mornings on "working tree has uncommitted changes to tracked files": the daily Cowork triage and digest rewrite files under bugs/ and features/, and once one of the three tracked ops files was touched the old gate counted it as blocking dirt. scripts/auto-bugfix.sh now (T1) excludes bugs/ and features/ from the dirty-tree gate so ops prose never blocks a code run while real tracked dirt outside those paths still aborts (D4); (T2) commits any ops dirt at preflight, on freshly synced main and before the branch is cut, as a pushed chore(ops) commit, with every failure path warning and continuing so hygiene never stalls the run; and excludes bugs/ and features/ from the auto commit so the PR is always product code (D1). The T2 ops commit is pushed to origin/main (main is unprotected) so it never rides into the auto branch diff, with a soft unwind if the push fails. CLAUDE.md gains the "Ops paths are committed, not carried" standing rule in both session rule sets (T5) and bugs/RUN-LOG.md records the .git/index.lock note (T6). Two direct-to-main chore commits followed the merge: c42abae gitignores the scrubber-skipped _cowork-scratch dirs so T2 can never commit un-scrubbed PII (the existing legal-draft folder holds real emails), and 0510ce8 finishes #214 by tracking the full ~156-file bug and feature tracker set (was 3), gated on scrub-pii.py --check clean (one new reporter registered as R6 in the gitignored key + two PostHog ids tokenized) and removal of five .write* scratch probes. Verified in isolation: gate stays clean on a tracked ops edit, blocks on a tracked non-ops edit, the fix-commit dry-run carries zero ops paths, and the auto-verdict-only case makes no empty ops commit. Full live wrapper confirmation is left to the next 05:00 run per the brief.

## 2026-09-09 - Auto-bugfix resilience: worktree isolation + prune + P1 escalator + diagnostics (feature)
- type: feature
- pr: #252
- branch: ops/auto-bugfix-resilience
- ids: none
- scope: auto-bugfix-resilience
- migration: none
- status: merged
- tsc: clean

scripts/auto-bugfix.sh now runs each session in a throwaway git worktree cut off origin/main (removed on every exit path, R1), so a dirty primary checkout or a parked feature branch can no longer block or contaminate the nightly bug run; the tracked-dirt abort from #251 is gone from the run path. .env.local and node_modules are symlinked into the worktree so the headless session and tsc resolve (R3), and the branch stamp gained seconds (R2). The #251 ops-commit is kept but now best-effort and conditional on the primary being on main, so origin/main still gets fresh trackers without disturbing a human session. Also: a stale-branch prune (local + remote auto/bugfix-* older than 14 days with no open PR, D3), an aged-P1 escalation email + escalated RUN-LOG row (3 days, debounced, D4), and startup env diagnostics (PATH, claude version, node, npm, gh, D5). Verified in isolation: a real worktree off origin/main ran tsc clean inside itself and tore down with zero leftovers; the escalator parser reads the real Build-this line and a 4-day-old brief triggers while age-0 does not; the prune selection matched the four abandoned remotes. A full --dry-run smoke confirmed the worktree lifecycle, the prune (cleared all 4 abandoned remote branches + 4 stale locals), the ops-commit conditional skip, T4 diagnostics, and R1 cleanup on the failure path. The dry-run's nested claude -p aborted on "OAuth session expired and could not be refreshed": that is a pre-existing environmental blocker (the claude CLI login on Jay's Mac, the same "claude headless run errored" class the RUN-LOG logged Aug 21-29), NOT a defect in this change, and it is now visible in the log thanks to T4. The 05:00 pipeline will not SHIP until the claude CLI is re-authenticated, a Jay step; the tree-blocking failure class is separately fixed. Known interaction: the session's SHIP-LOG pending entry is now written inside the disposable worktree and not persisted; auto bug fixes carry a BUG-NNN in the PR title so the Cowork git-log reconcile still closes them.

## 2026-09-09 - linestry-ops repo + ops_pii_map (the ops-layer cloud unlock) (feature)
- type: feature
- pr: #253
- branch: ops/linestry-ops-pii-map
- ids: none
- scope: linestry-ops-repo
- migration: applied - 20260909000001_ops_pii_map (CREATE TABLE public.ops_pii_map, RLS enabled no policies = service-role only; SAFE additive; applied via MCP + populated with 7 reporters/60 sessions)
- status: merged
- tsc: clean
...
## 2026-09-11 - Activation, retention, and the funnel scoreboard (feature)
- type: feature
- pr: #258
- branch: feat/activation-retention-scoreboard
- ids: none
- scope: activation-retention-scoreboard
- migration: applied - 20260910000001_activation_retention_scoreboard (SAFE additive: ADD COLUMN analytics_events.distinct_id, partial unique index analytics_events_one_activation_per_actor, two read-only SQL functions analytics_funnel_summary + analytics_cohort_retention; nothing against profiles; applied + verified via MCP BEFORE merge because captureServerEvent sends distinct_id unconditionally and a missing column fails every analytics write silently)
- status: merged
- tsc: clean

Defines an activated member as signed up AND posted >=1 story AND returned on a later calendar day (D1), checked off durable state via src/lib/activation.ts (maybeMarkActivated) from POST /api/stories and the daily-visit branch of GET /api/me; emitted as member_activated, exactly-once by a partial unique index (dup insert swallowed). Adds member_returned (day_index prop, never signup day). Adds analytics_events.distinct_id so pre-auth funnel steps count people not page loads, written by captureServerEvent, plus a conservative bot user-agent stamp (props.bot=true) at POST /api/track/event and an EXCLUDED_ACTOR_IDS constant (Jay's id), both filtered in the RPCs. New moderator-gated /admin/funnel (KPI row, 8-step funnel with step-to-step conversion, two-series daily chart, weekly cohort retention, content counts) backed by GET /api/admin/funnel + the two functions, plus a /admin chip. Two deliberate deviations from the drafted brief: (1) analytics_cohort_retention returns the last N cohorts that HAVE members, not a rolling N-calendar-week window, so A12's pre-June-2026 cohort actually shows against real data (verified back to 2026-04-20); (2) revoked EXECUTE from PUBLIC (not just anon/authenticated) since the default grant is PUBLIC. Verified: migration pre-flight/post-apply SQL clean, both RPCs return correct shaped jsonb, gate returns 307 (page) and 401 (api) unauth, funnel steps 1-3 read 0 on historical rows as expected. Deferred to Jay/post-deploy: A2-A8 activation/return smoke (need a live editor account + UTC-day boundary) and loading /admin/funnel on prod. Aside: pre-flight found 3 orphan auth users (auth.users with no profiles row, expected 0) - invisible to cohort retention by design, not this session's fix.

## 2026-09-11 - First-touch acquisition attribution, full build (feature)
- type: feature
- pr: #259
- branch: feat/funnel-attribution
- ids: none
- scope: funnel-attribution
- migration: applied - 20260910000002_funnel_attribution (SAFE additive: CREATE TABLE ref_codes + acquisition, indexes, RLS, comments; nothing against profiles; applied + verified via MCP before merge, soft gate only since every reader/writer degrades to a missing row; seeded the 'fnrad' code)
- status: merged
- tsc: clean

Completes the funnel-attribution brief on top of the episode-1 carve (#238, which shipped src/lib/attribution.ts, captureTouch wiring, and the signup_started/signup_succeeded stamps). Adds the durable half: two additive tables (ref_codes spoken campaign codes + acquisition one-row-per-member first-touch-write-once/last-touch, no profiles column so the signup path is unbreakable, D1); GET /r/[code] route handler that 302s (D6) to a code's destination with campaign params + ref appended and fires ref_code_hit, unknown/inactive codes 302 to onboarding with the code preserved (matched:false, never 404), destination_path run through safeReturnTo so a bad row cannot bounce off-site; POST /api/attribution/attach fire-and-forget durable write (always {ok:true}) using insert-then-update-last-only so first-touch is immutable at the DB level; cross-device carry via the existing pending_onboarding stash (magic link on a phone still attributes to the laptop), resolved in /auth/complete (localStorage-or-stash, attach on new-profile-only, stamp signup_succeeded + ftue_completed from the resolved touch, then clear); ftue_landed + ftue_completed now carry attributionProps (the two the carve skipped); /admin/users "Came from" column + source filter; one /privacy sentence (D12). Added attributionPropsFrom(stored) so the cross-device path stamps from the resolved touch; attributionProps() delegates to it (no behaviour change). Verified without auth: A2 (fresh first==last), A3 (immutability - the key one), A8 (/r/fnrad exact params, case-insensitive, trailing slash, foo=1 preserved), A9 (unknown -> matched:false), A10 (open-redirect guard, https:// and // stay on-site), A15 (privacy renders). Deferred to Jay/post-deploy (need auth or two devices): A11-A14 full signup + admin column render, A16 no-key. Decision flagged: LEGAL_EFFECTIVE deliberately NOT bumped (addition narrows nothing, per D12/T10). Note: localhost smoke wrote ~11 inert ref_code_hit test rows to prod analytics_events (dev server uses prod service key); not a scoreboard metric, left in place since analytics DELETE is GATED.

## 2026-09-11 - Pin search_path on the analytics scoreboard functions (chore)
- type: chore
- pr: #260
- branch: chore/pin-analytics-function-search-path
- ids: none
- scope: analytics-function-hardening
- migration: applied - 20260910000003_pin_analytics_function_search_path (SAFE: CREATE OR REPLACE the two #258 functions with set search_path='' + idempotent revokes; no data, no profiles; applied + verified via MCP before merge)
- status: merged
- tsc: clean
