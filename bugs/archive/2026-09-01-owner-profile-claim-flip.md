...
> **RESOLVED + CLOSED (September 6, 2026, attended).** Neither hydration/auth-flip mechanism this brief hypothesised fired. The actual cause was §4's third possibility, an identity mismatch: the reported URL pointed at a duplicate empty unclaimed ghost node, not the owner's real account. Closed by deleting the ghost, then fixing the class of bug via the duplicate-person-prevention pass (PRs #227-#231 + the Todd merge + the §9.2 orphan repair). See `bugs/2026-09-06-duplicate-person-creation-followup.md` and `features/duplicate-person-prevention-brief.md`. This brief is retained for the record; do not build from it. Follow-up 2 (replay S-49, how "Create A Story" reached the ghost page) remains open and separate.
