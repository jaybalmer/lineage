...
Reads:  <ops>/queues/bug-triage.md, <ops>/queues/NEXT-SESSION.md
Writes: <ops>/log/bug-queue.html
...
TRIAGE = OPS / "queues" / "bug-triage.md"
NEXT = OPS / "queues" / "NEXT-SESSION.md"
OUT = OPS / "log" / "bug-queue.html"
...
def parse_queue(md: str):
    ...
    for part in parts:
        ...
        # Brief = first "Brief at `<path>.md`" or "Added to `<path>.md`". The
        # rewritten triage writes brief paths under briefs/; older entries still
        # carry the legacy bugs/ prefix, and either may be prefixed linestry-ops/.
        # Match both so the dashboard links never silently go blank.
        brief = ""
        bm = re.search(r"(?:Brief at|Added to)\s+`?((?:linestry-ops/)?(?:briefs|bugs)/[^`\s]+\.md)`?", body)
        if bm:
            brief = bm.group(1)
...
<div class="wrap">
  <header>
    ...
    <p class="sub" id="sub">Snapshot. Source: linestry-ops/queues/bug-triage.md.</p>
  </header>
...
<script>
  ...
  function render(){
    ...
    document.getElementById("sub").textContent = "Snapshot as of "+SNAPSHOT+". Source: linestry-ops/queues/bug-triage.md. Lead: "+LEAD+". Regenerated by the daily triage.";
    ...
    document.getElementById("foot").innerHTML =
      "This is a snapshot dashboard generated from linestry-ops/queues/bug-triage.md by the daily triage (bugs/render-queue.py). Next free id: "+NEXTID+".";
  }
