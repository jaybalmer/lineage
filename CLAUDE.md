...
- **Ops paths are committed, not carried.** `bugs/` and `features/` hold the trackers and briefs Cowork rewrites daily. The 05:00 wrapper commits any dirt there as `chore(ops)` before it starts, and the auto PR never contains a file from either directory. If you edit a tracker in a session, commit it in its own `chore(ops)` commit rather than folding it into a fix or feature commit.
