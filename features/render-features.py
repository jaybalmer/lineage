...
Reads:  <ops>/queues/feature-queue.md, <ops>/queues/NEXT-FEATURE.md
Writes: <ops>/log/feature-queue.html
...
QUEUE = OPS / "queues" / "feature-queue.md"
NEXT = OPS / "queues" / "NEXT-FEATURE.md"
OUT = OPS / "log" / "feature-queue.html"
...
def parse_lead_brief(next_md: str) -> str:
    # The rewritten digest writes brief paths under briefs/; older entries still
    # carry the legacy features/ prefix, and either may be prefixed linestry-ops/.
    # Match both so the lead card is still flagged after the cutover.
    idx = next_md.find("Build this")
    if idx != -1:
        m = re.search(r"`((?:linestry-ops/)?(?:briefs|features)/[^`]+\.md)`", next_md[idx:idx + 400])
        if m:
            return m.group(1)
    return ""
...
<div class="wrap">
  <header>
    ...
    <p class="sub" id="sub">Snapshot. Source: linestry-ops/queues/feature-queue.md.</p>
  </header>
...
<script>
  ...
  function render(){
    ...
    document.getElementById("sub").textContent = "Snapshot as of "+SNAPSHOT+". Source: linestry-ops/queues/feature-queue.md. Lead: "+LEAD+". Regenerated by the morning digest.";
    ...
    document.getElementById("foot").innerHTML =
      "This is a snapshot dashboard generated from linestry-ops/queues/feature-queue.md by the morning digest (features/render-features.py). Features have no BUG id; they reconcile off linestry-ops/log/SHIP-LOG.md type:feature + git.";
  }
